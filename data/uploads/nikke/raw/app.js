...
const BATTLE_DEFAULT = {
  ...
  // (실험실) 버스트 게이지 실누적 — "fixed"면 위 두 값(첫 버스트·재충전)이 사이클을
  // 정하고, "accumulate"면 **쏜 히트가 게이지를 만들어** 100%에 1단계가 열린다.
...
function cleanPhases(list) {
  return (Array.isArray(list) ? list : [])
    .map((p) => {
      ...
      return { kind, t0: tick(p?.t0), t1: tick(p?.t1) };
    })
    .filter((p) => p.t1 > p.t0)
    .sort((a, b) => a.t0 - b.t0)
    .slice(0, PHASE_MAX);
...
function battlePayload(d = null) {
  ...
  // (실험실) 실누적 게이지. **켰을 때만** 실어 보낸다 — "fixed"를 매번 실으면 안
...
const ACC_ORDER = ["verified", "verifying", "unsupported", "bug"];
const ACC_LABEL = { verified: "표시 없음", verifying: "확인중",
...
function fbcLabel(d, orders) {
  ...
  return orders.map((o) => (o.order.length
      ? T("{v}버 {name}", { v: o.stage, name: o.order.join(" → ") })
      : T("{v}버 없음", { v: o.stage }))            // 그 단계를 통째로 금지한 상태
    + (o.bans.length ? T(" ({v} 금지)", { v: o.bans.join(" · ") }) : "")).join(" · ");
}
...
function currentPreset(name, kind, withBoss = true) {
  const union = modeNow() === "union";
...
function savePreset(want, kind, withBoss = true) {
  ...
  const next = currentPreset(name, kind, withBoss);
...
function unionCodes(decks) {
  ...
  const abbr = decks.map((d) => CODE_ABBR[d.weak]);
...
function openBossLinkSheet(got) {
  ...
  if (okBtn) okBtn.onclick = () => {
    ...
    dlg.close();
    flashStatus(r.err || T("회차 보스 기본값 {n}개를 가져왔습니다 — 세 줄은 그대로입니다.",
                           { n: r.n }));
  };
  const x = $("#bosslink-x");
...
function sharePickBox(srcIdx, names, host, extra = null) {
  ...
  if (missing.length) {
    box.append(el("p", "share-pick-note",
      T("내 계정에 없는 {length}명은 빈 자리로 들어갑니다 — {v}.", { length: missing.length, v: missing.join(" · ") })));
  }
...
function bindChrome() {
  ...
  $("#calc-wip-log").onclick = openCalcLog;
  $("#calc-wip-notice").onclick = openNotice;
  $("#calclog-x").onclick = () => $("#calclog-sheet").close();
...
/** 공지를 **지금 연다** — «다시 보지 않기»를 눌렀든 말든. 상단 배너의
 *  «업데이트 내역»이 부른다. 한 번 껐다고 다시 볼 길이 없으면 안 된다. */
function openNotice() {
...
/** 상단 배너의 «로직 변경 내역» — 계산 로직 변경만 쌓는 목록(CALC_CHANGES)을 그린다. */
function openCalcLog() {
  const box = $("#calclog-list");
  box.textContent = "";
  for (const sec of CALC_CHANGES) {
    box.append(el("h4", "notice-date", sec.date));
    let ul = null;
    for (const item of sec.items) {
      if (item === "") { ul = null; continue; }
