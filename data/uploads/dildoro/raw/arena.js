...
(() => {
  ...
  /** 저장해 둔 편성에 **지금 로스터에 없는 이름**이 남아 있을 수 있다(니케 이름이
   *  바뀌거나 자료에서 빠지면). 그대로 보내면 서버가 «모르는 니케»로 되돌려 보내
   *  화면이 통째로 막힌다 — 그 자리만 비우고 한 번 알려 준다. */
  let droppedNames = [];
  const prune = (d) => {
    if (!d || !Array.isArray(d.names)) return d;
    d.names = d.names.map((n) => {
      if (!n || byName.has(n)) return n || null;
      droppedNames.push(n);
      return null;
    });
    return d;
  ...
  const load_ = () => {
    try {
      ...
      if (raw && raw.def && raw.atk) { prune(raw.def); prune(raw.atk); return raw; }
    } catch { /* 깨졌으면 새로 */ }
    return { def: blank(), atk: blank(), skill: 10, res: null };
  };
  const save_ = () => {
    scheduleTier();          // 편성이 바뀌었다는 뜻이다 — 속도도 다시 묻는다
    try {
  ...
  /** 이 편성이 **풀버스트에 갈 수 있나**. 엔진은 빈 단계를 건너뛰지 않는다(v3.6,
   *  코어 세션 2026-09-01) — Ⅰ·Ⅱ·Ⅲ가 한 명씩 없으면 사슬이 거기서 끊긴다. 실제
   *  게임이 그렇다. 「A」단계(어느 자리든 메우는 니케 — 레드 후드)는 빈 자리를 채운다.
   *  반환: 그래도 비는 단계들(없으면 빈 배열). */
  function burstGap(names) {
    const have = { 1: 0, 2: 0, 3: 0 };
    let wild = 0;
    for (const n of names.filter(Boolean)) {
      // **배지에 보이는 값**을 본다(`burst`). 솔로 계산이 쓰는 `burstStageOf`는 「A」를
      // 한 단계로 못 박아 두는데(레드 후드 → 3버), 아레나 엔진은 「A」를 빈 자리에
      // 끼워 넣는다 — 여기서 못 박으면 없는 경고가 뜬다.
      const st = String(byName.get(n)?.burst ?? "?");
      if (st === "1" || st === "2" || st === "3") have[st]++;
      else wild++;                       // 「A」 — 어느 단계든 메운다
    }
    const miss = [1, 2, 3].filter((k) => !have[k]);
    return miss.slice(wild);             // 와일드가 앞에서부터 메운 뒤 남는 것
  }

  const ROMAN = { 1: "Ⅰ", 2: "Ⅱ", 3: "Ⅲ" };

  /** 엔진이 준 **편성 속도**. `{tier, sec, chainOk}` — 저장하지 않는다(편성이 바뀌면
   *  바로 헌 값이 된다). 엔진 v3.6의 `tier_only`는 전투를 안 돌려서 0.5ms다. */
  const TIER = { atk: null, def: null };
  let tierTimer = 0, tierSeq = 0;
  /** 엔진이 `tier_only`를 아는가. **모르면 전투를 통째로 돌려 버린다**(0.3초·서버 CPU) —
   *  그래서 한 번 겪으면 이 화면에서는 다시 안 묻는다. 새로 고치면 다시 재 본다. */
  let tierMode = true;

  /** 편성이 바뀔 때마다 부른다 — 몰아서 한 번만 나간다. */
  function scheduleTier() {
    if (!tierMode) return;
    clearTimeout(tierTimer);
    tierTimer = setTimeout(askTier, 250);
  }

  async function askTier() {
    const one = (d) => ({ names: d.names.filter(Boolean), level: d.level });
    const atk = one(A.atk), def = one(A.def);
    if (!atk.names.length && !def.names.length) {
      TIER.atk = TIER.def = null; paintTier("atk"); paintTier("def"); return;
    }
    // 서버는 **양쪽 다** 니케가 있어야 받는다. 속도는 편성 하나만으로 재는 값이라
    // 한쪽이 비었으면 반대쪽을 두 번 보내고 채워진 쪽 답만 쓴다.
    const body = {
      tier_only: true,
      attack: atk.names.length ? atk : def,
      defense: def.names.length ? def : atk,
    };
    const seq = ++tierSeq;
    try {
      ...
      if (seq !== tierSeq) return;          // 그 사이 편성이 또 바뀌었다
      if (j.error) throw new Error(j.error);
      // **속도만 묻는 답인지**를 `burst_chain_ok`로 가른다 — 옛 엔진은 `tier_only`를
      // 모르고 전투를 통째로 돌려 답한다(그 답에도 `tier`는 있다). 그때는 이 화면에서
      // 다시 묻지 않는다.
      const chainOf = (o) => (o && typeof o.burst_chain_ok === "boolean" ? o.burst_chain_ok : null);
      if (chainOf(j.attack) === null && chainOf(j.defense) === null) {
        tierMode = false;
        TIER.atk = TIER.def = null;
        paintTier("atk"); paintTier("def");
        return;
      }
      const take = (o) => (o && o.tier
        ? { tier: String(o.tier), sec: Number(o.deck_gauge_sec) || 0, chainOk: chainOf(o) }
        : null);
      TIER.atk = atk.names.length ? take(j.attack) : null;
      TIER.def = def.names.length ? take(j.defense) : null;
    } catch {
      // 옛 엔진(501)·네트워크 끊김 — 조용히 접는다. 사슬 경고는 우리 계산이 대신한다.
      if (seq !== tierSeq) return;
      TIER.atk = TIER.def = null;
    }
    paintTier("atk"); paintTier("def");
  }

  /** 편성 판의 **속도 딱지**와 **버스트 경고**만 다시 그린다 — 판 전체를 다시 그리면
   *  레벨 칸에 있던 손가락이 튕긴다. */
  function paintTier(side) {
    const box = document.getElementById(side === "def" ? "arena-def" : "arena-atk");
    if (!box) return;
    const d = sideOf(side);
    const filled = d.names.filter(Boolean).length;
    const t = TIER[side];

    const chip = box.querySelector(".arena-tier");
    if (chip) {
      chip.hidden = !t;
      if (t) {
        chip.textContent = t.tier;
        chip.title = t.sec
          ? T("첫 풀버스트까지 {v}초 — 상대 없이 편성만으로 잰 값입니다. 실전에서는 맞으면서 게이지를 얻는 니케(자칼 등)가 있으면 더 빨라집니다.", { v: t.sec.toFixed(1) })
          : T("팀 속도 등급 — 편성만으로 잰 값입니다.");
      }
    }

    // 사슬 판정은 **엔진 말이 우선**이다(레드 후드 같은 예외까지 엔진이 본다).
    // 엔진이 아직 답을 안 했거나 못 하는 서버면 우리 계산으로 대신한다.
    const gap = burstGap(d.names);
    // 엔진이 사슬을 봐 줬으면 그 말이 옳다. 안 봐 줬으면 우리 계산이 대신한다.
    const bad = filled > 0 && (t && t.chainOk !== null ? t.chainOk === false : gap.length > 0);
    const warn = box.querySelector(".arena-burst-warn");
    if (warn) {
      warn.hidden = !bad;
      const line = gap.length
        ? T("버스트 {v}단이 없어 풀버스트에 못 갑니다.", { v: gap.map((k) => ROMAN[k]).join("·") })
        : T("버스트 사슬이 끊겨 풀버스트에 못 갑니다.");
      warn.querySelector("span").textContent = line;
    }
  }
  ...
  function renderSide(side) {
    ...
    // 편성 속도 등급(2RL·2.5RL(5SG)…) — 값은 `paintTier`가 채운다.
    const tchip = el("span", "arena-tier");
    tchip.hidden = true;
    head.append(tchip);
    ...
    // 버스트 사슬이 끊기면 그 자리에서 알려 준다 — 결과를 보고 나서야 «왜 한 번도
    // 안 터졌지»를 찾게 하면 안 된다.
    const w = el("div", "arena-burst-warn");
    w.hidden = true;
    w.append(el("em", null, "!"));
    w.append(el("span"));
    box.append(w);
  ...
  function renderRes() {
    ...
    for (const side of ["attack", "defense"]) {
      ...
      // 팀 속도 등급(2RL·2.5RL(5SG)…) — 이 판의 결과가 아니라 **편성 자체의 속도**다.
      // 커뮤니티에서 덱마다 붙여 부르는 그 표기다(엔진 v3.6).
      //
      // **«상대와 무관»이라고 쓰면 안 된다.** 등급 자체는 상대 없이 재지만, 실전 만충은
      // 상대에 따라 달라진다 — 자칼처럼 «맞으면 게이지를 얻는» 니케가 있으면 상대가
      // MG·AR일 때 더 빨리 찬다(코어 세션 확인 2026-09-01: 같은 편성이 상대에 따라
      // 75프레임 대 65프레임, 자칼을 빼면 두 상대 모두 113프레임으로 같아졌다).
      if (t.tier) {
        const tier = el("span", "arena-tier", String(t.tier));
        tier.title = T("팀 속도 등급 — 상대 없이 편성만으로 잰 첫 풀버스트 속도입니다. 실전에서는 맞으면서 게이지를 얻는 니케가 있으면 더 빨라집니다.");
        head.append(tier);
      }
      panel.append(head);
  ...
  const EV_COLOR = {
    ...
    cover_break: "#8a5fd0", death: "#e0344f", revive: "#57d98a",
  };
  const EV_LABEL = {
    burst_cast: "버스트 시전", full_burst: "풀버스트", gauge_full: "게이지 만충",
    cover_break: "엄폐 파괴", death: "전투 불능", revive: "부활",
  };
  ...
  function ensure() {
    ...
    renderSide("def");
    renderSide("atk");
    scheduleTier();          // 들어오자마자 한 번 — 저장해 둔 편성의 속도를 채운다
    if (droppedNames.length) {
      msg(T("지금 자료에 없는 니케라 편성에서 비웠습니다 — {v}",
        { v: droppedNames.join(" · ") }), "warn");
      droppedNames = [];
      save_();
    }
    if (wired) return;
